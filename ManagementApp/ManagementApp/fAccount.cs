...
namespace ManagementApp
{
    public partial class fAccount : Form
    {
        ...
        void loadcbbDevice()
        {
            ...
            var max = db.StaffAccounts.Where(bien => bien.UserSA == user).FirstOrDefault();
            string mnv = (max.idSA).ToString();
            try
            {
                var devices = db.Devices
                                 .Where(d => d.idSA == mnv)
                                 .Select(d => new { d.idDevice, d.nameDevice })
                                 .ToList();
                cbbDevice.DataSource = devices;
                cbbDevice.DisplayMember = "nameDevice";
                cbbDevice.ValueMember = "idDevice";
            }
            catch (Exception ex)
            {
        ...
        private void btnAdd_Click(object sender, EventArgs e)
        {
            ...
            {
                var getiddv = int.Parse(cbbDevice.SelectedValue.ToString());
